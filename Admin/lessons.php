...
<div class="col-sm-9 mt-5  mx-3">
  ...
  $result = pg_query($conn,"SELECT course_id FROM course");
  while($row = pg_fetch_assoc($result)){
    if(isset($_REQUEST['checkid']) && $_REQUEST['checkid'] == $row['course_id']){
      //$sql = "SELECT * FROM course WHERE course_id = {$_REQUEST['checkid']}";
      $result = pg_query($conn,"SELECT * FROM course WHERE course_id = {$_REQUEST['checkid']}");
      ...
      if(($row['course_id']) == $_REQUEST['checkid']){ 
        ...
        <?php
          ...
          $result = pg_query($conn,"SELECT * FROM lesson WHERE course_id = {$_REQUEST['checkid']}");
        ...
        if(isset($_REQUEST['delete'])){
         ...
         $result=pg_query($conn,"DELETE FROM lesson WHERE lesson_id = {$_REQUEST['id']}");
         if(($result) === TRUE){
